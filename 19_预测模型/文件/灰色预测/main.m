...
if size(x0,1) == 1 % 此时为行数=1 -> 则说明是行向量
    x0 = x0';
end
if size(year,1) == 1 % 同理
    year = year';
...
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    ...
    % .+运算符, 就是对应位置上的元素进行计算
    % 因为x1是新序列,是x0累加后得到的序列, x0(k)是原始序列
    
    % 画出光滑度的图形，并画上0.5的直线，表示临界值
    figure(2)
    plot(year(2:end),rho,'o-',[year(2),year(end)],[0.5,0.5],'-'); % 画出光滑度曲线
    grid on;
    text(year(end-1)+0.2,0.55,'临界线')   % 在坐标(year(end-1)+0.2,0.55)上添加文本, 表示在那个位置上加上文本
    set(gca,'xtick',year(2:1:end))  % 设置x轴横坐标的间隔为1
    xlabel('年份');  ylabel('原始数据的光滑度');  % 给坐标轴加上标签
    
    % 然后计算在光滑度在(0, 0.5)这个区间范围内的占比.
    ...
    Judge = input('你认为可以通过准指数规律的检验吗？可以通过请输入1，不能请输入0：'); % 用来接收是0还是1
    if Judge == 0
...
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    if  n > 4  % 数据量大于4时，将数据分为训练组和试验组(根据原数据量大小n来取，n为5-7个则取最后两年为试验组，n大于7则取最后三年为试验组)
        ...
        train_x0 = x0(1:end-test_num);  % 训练数据 % 后三期为试验组的数据, 那么前几期就是训练组的数据
        % 打印一下
        disp('训练数据是: ')
        disp(mat2str(tarain_x0'))  % mat2str可以将矩阵或者向量转换为字符串显示, 这里加一撇表示转置，把列向量变成行向量方便观看
        ...
        test_year = year(end-test_num+1:end);  % end是一种预设值
        figure(3)
        ...
        % 我们要拼接字符串, 就要使用元胞数组, 这是一种可以容纳任何元素的数组
        disp(strcat('因为',Model(choose),'的误差平方和最小，所以我们应该选择其进行预测')) % 索引选择. % 打印结果
        disp('------------------------------------------------------------')
        
        %% 上面已经得到了SSE最小的那个进行
        %% 选用误差最小的那个模型进行预测
        predict_num = input('请输入你要往后面预测的期数： '); % 接受要预测期数
        ...
        % 得到一系列的结果, 使用传统的GM(1,1)模型得到其他的统计量

        % % 判断我们选择的是哪个模型，如果是2或3，则更新刚刚由模型1计算出来的预测结果
        % 对result进行更新 -> 更新结果, 但是其他的统计量不变.
        if choose == 2
