...
% sim(net, new_X(1,:)) % 要使用列向量。
% 错误使用 network/sim (line 266)
% Input data sizes do not match net.inputs{1}.size.
% net.inputs{1}.size
 
% 这里要注意，我们要将指标变为列向量，然后再用sim函数预测
sim(net, new_X(1,:)') % 这是第一行行向量转置称为列向量
% sim函数就是用来使用神经网络进行仿真或者拟合的

% 写一个循环，预测接下来的十个样本的辛烷值
predict_y = zeros(10,1); % 初始化predict_y -> 预测保存的结果
for i = 1: 10
    result = sim(net, new_X(i,:)'); % 每次预测一行（一个样本）
    predict_y(i) = result;  % 保存结果。
end
disp('预测值为：')
disp(predict_y)
