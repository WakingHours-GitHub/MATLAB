...
%% solve函数 -> 一个最基本的求解方程组的函数.
% solve(y, x)
% y是方程表达式
% x是需要求解的变量, 只有一个变量, x可以省略
...
syms u v a
eqn = [2*u + v == a, u - v == 1];
answ = solve(eqn, [u, v]) % 指定对那个变量求解，向量包含需要求解的变量
% answ：   包含以下字段的 struct:
% 结构体引用成员变量就是用 . 运算符
answ.u
answ.v
...
% 警告. 不能使用solve解决符号表达式, 返回一个数值型的近似解
% solve()函数的求解能力是有限的

% 画图看看 
fplot(sin(x), [-2 2])  % fplot函数可绘制表达式的图形, 默认是对x的范围
hold on; % 在原来的脚本上继续画图
fplot(x^2 - 1, [-2 2]) 
...
%% vpasolve函数求解
% vpasolve(y, range)函数,
% y是待求解的函数, 可以传入方程组, 也用向量表示
% range是范围，用向量表示
% 用vpasolve函数指定求[0 2]上的解
syms x
eqn = sin(x) == x^2 - 1;
vpasolve(eqn, x, [0 2]) % 求表达式在0~2范围上的解
vpasolve(eqn, x, [-1 0])

vpasolve(eqn, x, [-10 10]) % 这样也仅仅会返回一个解.
% vpasolve returns all solutions only for polynomial equations. only for多项式方程
...
vpasolve(eqn, x, 'random', true) % 重复的调用, 有可能就会搜索到全部解
...
% 画图看看
% 上面两个方程可以看成是隐函数, 所以我们只需要使用隐函数的画图函数,就可以画出图形
...
hold on
plot(answ_x, answ_y,'ko', 'MarkerSize',10)   % 不转换也可以直接画图
% plot(double(answ_x), double(answ_y),'ko', 'MarkerSize',10)  % double()可以将符号变量转换成为数值型
% text(answ_x, answ_y, '说明')  % 你也可以加一个文本
% double可以将我们的符号变量转换为数值变量, 然后画图



%% fsolve函数（求解功能最为强大哦）
% fsolve是Matlab优化工具箱中的一个函数，可专门用来求解特别复杂的方程和方程组
% 使用fsolve函数需要有一个函数，来表示要求解的方程
x0 = [0,0];  % 初始值
