...
% z的中间有一个点0/0 -> 得到的是Nan -> 这个点在mesh中是不体现的
% 如何解决: 
% 1. 手动修改，让=0的那个位置利用极限算出数值，然后修改
% 2. 在z=f(x,y)的后面加上一个非常小的数字。eps就是一个很好的选择。 eps表示matlab浮点数的精度
mesh(x,y,z)
xlabel('x轴');  ylabel('y轴');  zlabel('z轴');  % 加上坐标轴的标签
axis vis3d % 冻结屏幕高宽比，使得一个三维对象的旋转不会改变坐标轴的刻度显示
