...
% edit peaks -> 编辑peaks函数
% peaks是matlab展示用作的命令, 直接生成一个图像.
...
colorbar % 显示颜色栏，也可以手动插入colorbar, 在右边显示色阶
xlabel('x轴');  ylabel('y轴');  % 加上坐标轴的标签

%% 手动选择分层的高度
%% contour(x,y,z,levels) 若想得到固定的n个高度的等高线，将levels可以设置为n元行向量，其中向量中的值为高度值。
maxz = max(max(z)) % 或者是max(z(:)), z(:)是所有的z元素按照列遍历, 然后再求max, 只需要一个max就可以了
minz = min(min(z))
levels = linspace(minz,maxz,10)  % 从最小值到最大值，等分成10个点 linspace()函数  
contour(x,y,z,levels,'ShowText','on','LineWidth',1)  % n = levels, 为高度等级
% 最小值或者最大值可能只是一个点, 所以等高线中显示不出来，因为Matlab会帮我们自动调整\

xlabel('x轴');  ylabel('y轴');  % 加上坐标轴的标签

% 思考如果只想画出高度为3的单等高线怎么办?
% 就是在n=[h h] 这样范围就是h到h, 这样就可以显示单等高线
