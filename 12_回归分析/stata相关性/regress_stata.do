...
import excel "课堂中讲解的奶粉数据.xlsx", sheet("Sheet1") firstrow // 文件与stata代码在同一根目录下可以直接导入

// 定量变量的描述性统计 
summarize 团购价元 评价量 商品毛重kg
// 简写:sum
// 结果不要直接放在论文中，粘贴到Excel表格上去，绘制成三线图

// 定性变量的频数分布，并得到相应字母开头的虚拟变量gen()函数作用
tabulate 配方,gen(A)
tabulate 奶源产地 ,gen(B)
...
est store m1 // 这个是把上面的回归分析，生成一个名为m1的模型
reg2docx m1 using m1.docx, replace // 这个命令是将m1这个模型生成一个m1.docx文档
// *** p<0.01  ** p<0.05 * p<0.1 // 生成的表格下面要标注显著性说明

// Stata会自动剔除多重共线性的变量
regress 评价量 团购价元 商品毛重kg A1 A2 A3 B1 B2 B3 B4 B5 B6 B7 B8 B9 C1 C2 D1 D2 D3 D4 D5 E1 E2 E3 E4 F1 F2 G1 G2 G3 G4
est store m2 // 继续对上面的回归生成模型
reg2docx m2 using m2.docx, replace // 将模型生成为docx文档

// 得到标准化回归系数: regress ,beta
// 看哪个自变量对y影响最大, 绝对值越大就代表影响越大
...
// 画出残差图, 先reg回归, 然后在画出残差图       
regress 评价量 团购价元 商品毛重kg A1 A2 A3 B1 B2 B3 B4 B5 B6 B7 B8 B9 C1 C2 D1 D2 D3 D4 D5 E1 E2 E3 E4 F1 F2 G1 G2 G3 G4

rvfplot 
// 残差与拟合值的散点图
graph export a1.png ,replace // 保存该散点图

// 残差与自变量团购价的散点图
rvpplot  团购价元 // 残差与自变量的散点图
...
// H0：贝特1~n = 0, 也就是不存在异方差
// 当p < a(显著性水平), 也就是我们拒绝原假设
