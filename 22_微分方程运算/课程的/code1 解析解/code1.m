...
% 符号运算：
syms y(x) % 定义了一个符号函数， y是符号函数，x是符号变量，即y是因变量，x是自变量
eqn = (y - diff(y,x) == 2*x);    % 注意原来方程中的“=”一定要改成“==” (列出微分方程表达式)
dsolve(eqn) % 直接传入方程
...
Dy = diff(y,x); % 定义变量Dy为y的一阶导数, 需要定义一下
...
% 也可以用: 直接用一个变量来接受, 例如: A = dsolve(... ) 然后返回的是一个结构体
% 我们可以用A.属性的方式来查看结果
...
% 警告: Explicit solution could not be found. -> 求不出来.
