...
% 我们使用上节课学习的dsolve函数进行求解

%% 调用ode45函数求解微分方程df2，自变量为x，范围为[0,4*pi] ; 因变量为y1,y2和y3，初始值: y1(0)=0,y2(0)=y3(0)=1 
[x, y] = ode45('df2', [0 4*pi], [0 1 1]);  % 这里的y是一个有3列的矩阵哦！
plot(x, y(:,1), 'o', x, y(:,2), '*', x, y(:,3), '+')  % 绘图
legend('y1','y2','y3')  % 加上标注, 一一对应, 与你画图的顺序是对应的
axis([0, 4*pi, -inf, +inf])  % 设置横坐标范围为0-4pi，纵坐标范围不需要设置，写成-inf到+inf

% 然后可以选择导出图像, 选择渲染->分辨率,然后导出即可

% % 注意：代码文件仅供参考，一定 不要直接用于自己的数模论文中
% % 国赛对于论文的查重要求非常严格，代码雷同也算 作抄袭
% % 视频中提到的附 件可在售后群（购买后收到的那个无忧自动发货的短信中有加入方式）的群文件中下载。包括讲义、代码、我视频中推荐的资料等。
