...
% 其中fitresult是模型的一些结果, 包括, 模型, 估计的参数
% gof是模型评估的指标, 包括sse, r_square 等等.
% 他们两个都是结构体类型

t = 2001: 2030; % 预测的时间
xm = fitresult.xm; % 结构体.属性
r = fitresult.r;
y_pred = xm./(1+(xm./3.9-1).*exp(-r.*(t-1790)));  % 计算预测值（注意这里要写成点乘和点除）
hold on
% 预测未来三十年的
plot(year, population, 'b-', t, y_pred, 'r+')
